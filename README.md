...
| 클래스명 | 설명 |
|---------|------|
| **FileItem** | 파일 경로, 저장된 해시값, 무결성 상태를 저장하는 데이터 모델 |
| **HashEngine** |  SHA-256 알고리즘으로 파일 해시값 생성 |
| **FileManager** |  FileItem 목록 관리 및 .txt 파일로 데이터 저장/로드 |
| **IntegrityChecker** |  현재 해시값 계산 및 저장값 비교를 통한 무결성 판별 |
| **MainFrame** | JFrame 기반 GUI 메인 윈도우 |
| **Main** | 애플리케이션 진입점 |
